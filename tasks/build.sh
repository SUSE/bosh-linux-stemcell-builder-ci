...
sudo --preserve-env --set-home --user ubuntu -- /bin/bash --login -i <<SUDO
  ...
  mkdir -p tmp
  cp ../os-image/bosh-$OS_NAME-$OS_VERSION-os-image.tgz ./tmp/base_os_image.tgz
  bundle exec rake stemcell:build_with_local_os_image[$IAAS,$HYPERVISOR,$OS_NAME,$OS_VERSION,$PWD/bosh-linux-stemcell-builder/tmp/base_os_image.tgz,$build_number]
...
if [ -e bosh-linux-stemcell-builder/tmp/*-raw.tgz ] ; then
  ...
  meta4 file-set-url --metalink="$meta4_path" --file="${raw_stemcell_filename}" "https://s3.amazonaws.com/concourse-bosh-stemcell-dev/${IAAS}/${raw_stemcell_filename}"
fi
